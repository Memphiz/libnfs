...
int rpc_nfs_read_async(struct rpc_context *rpc, rpc_cb cb, struct nfs_fh3 *fh, off64_t offset, size_t count, void *private_data)
{
	struct rpc_pdu *pdu;
	READ3args args;
...
int rpc_nfs_write_async(struct rpc_context *rpc, rpc_cb cb, struct nfs_fh3 *fh, char *buf, off64_t offset, size_t count, int stable_how, void *private_data)
{
	struct rpc_pdu *pdu;
	WRITE3args args;
